...
class SingleMotor(_BasicDevice):
    ...
    def _send_motor_command(self, msg: bytes, result_class, blocking: bool, *, motor_idx: int | None = None):
        ...
        response = self.send(msg, blocking=blocking)
    ...
    def _send_motor_command_with_speed(self, motor_idx: int, speed, command_msg: bytes, result_class, blocking: bool):
        ...
        response = self.send(*messages, blocking=blocking_plan)
...
class Oled(_BasicDevice):
    def __init__(self):
        super().__init__()
        self.search_name = 'Oled'
        self.product_id = PRODUCT_GROUP_DEVICE_OLED
        self.sensor = OledNotification(0)

    @property
    def status(self):
        return self.sensor.status

    def print_text(self, text, scroll=True):
        import struct
        encoded_text = text.encode('ascii')
        payload = struct.pack(f"<B{len(encoded_text)}s", 0x02, encoded_text)
        self._send_commands(payload, blocking=False)
        if scroll:
            self._send_commands(struct.pack("<B", 0x03), blocking=False)
            
    def write_array(self, data_array):
        import struct
        payload = struct.pack(f"<B{len(data_array)}s", 0x01, bytes(data_array))
        self._send_commands(payload, blocking=False)
