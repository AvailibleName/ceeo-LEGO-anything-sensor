...
# OledNotification
# =====================================================
class OledNotification:
    FormatString = "<B"

    def __init__(self, status=1):
        self.status = status
        self.header = Header(OLED_NOTIFICATION)
...
def device_notification_parser(notification_obj):
    ...
    while deviceDataLength > 0:
        ...
        elif notification_type == OLED_NOTIFICATION:
            next_notification = OledNotification.Deserialize(deviceData[1:])
            notification_size = SizeUtil.getMsgSize(OledNotification)
        else:
