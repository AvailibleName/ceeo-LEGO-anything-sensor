...
class BasicBLE(BLETransport):
    ...
    def __init__(self, *, shutdown_callback: Optional[Callable[[], None]] = None):
        ...
        # Thread-safe cancellation flag.  Set from the main thread via
        # request_cancel(); polled from the BLE thread inside scan/connect
        # loops at ~0.1 s intervals. Cleared by the worker loop before each
        # new request dispatch (see background_worker.py).
        self._cancel_event = threading.Event()

    def request_cancel(self):
        """Request cancellation of any in-progress scan/connect operation.

        Thread-safe: can be called from any thread (typically the main thread
        via Worker.cancel_current_operation)."""
        self._cancel_event.set()

    def reset_cancel(self):
        """Clear cancellation request state prior to a new operation.

        Called by the worker loop right before dispatching a request so that
        stale cancel state from a previous operation does not leak."""
