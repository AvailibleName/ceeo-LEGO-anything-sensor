...
# Polling interval while waiting for BLE connection to be established
_CONNECT_POLL_INTERVAL_S = 0.05
...
class _BasicDevice:
    ...
    def __init__(self, device_name="any", device_mac="any", card_color=-1, card_serial=-1, callback=None):
        self._Background_thread_start()
        atexit.register(self._Background_thread_end)
        ...
        # used by connect() to wait for sensor values to be initialized.
        self._first_notification_ready = threading.Event()

        # Batch mode state (guarded by _batch_lock for thread safety)
    ...
    def _validate_thread(self, name):
        worker_thread = self.my_worker.ble_thread
        if worker_thread is not None and threading.current_thread() is worker_thread:
            raise RuntimeError(f"Function '{name}' cannot be called from the callback thread")

    def _reset_batch_state_locked(self):
        """Reset batch mode state. Caller must hold _batch_lock."""
        self._batch_mode = False
        self._batch_buffer = []
        self._batch_size = 0
    ...
    def _reset_batch_state(self):
        """Reset batch mode state under lock."""
    ...
    def _Background_thread_start(self):
        """Start the background worker thread that handles BLE I/O.
        Call before using any hardware APIs."""
        my_worker.start_thread()

    def _Background_thread_end(self):
        """Stop the background worker thread and release BLE resources.
        Safe to call when finished interacting with hardware."""
        my_worker.close_thread()

    def _scan_callback(self, devices):
        # Callback from worker thread after scan completes; store results.
        self.device_list = devices or []
        self.scanning = False

    def _ask(self, topic, message = None, message2 = None, message3 = None, message4 = None):
        worker_thread = self.my_worker.ble_thread
        if worker_thread is not None and threading.current_thread() is worker_thread and topic != 'send': # 'send' is allowed here, since a special check is done in _send_commands():
            raise RuntimeError("This function cannot be called from the callback thread")
        payload = {}
        payload['topic'] = topic
        payload['msg'] = message
        ...
        self.my_worker.put_request(payload)
    ...
    def _send_commands(self, *messages , blocking=True):
        ...
        # Size accounting: serialized messages are concatenated directly into a
        # single BLE write (no per-message framing or length prefix on the wire),
        # so summing len(msg) accurately tracks the payload size against
        # maxPacketSize.
        if self._batch_mode:
            with self._batch_lock:
                for msg in messages:
        ...
        worker_thread = self.my_worker.ble_thread
        if worker_thread is not None and threading.current_thread() is worker_thread and blocking_check:
            raise RuntimeError("This function cannot be called from callback thread with 'blocking=True'")
        ...
        for i, msg in enumerate(messages):
            ...
            response_id = msg[0] + 1
            # print(f"Sending message ID {msg[0]}, expecting response ID {response_id}")
            ...
            # responses are matched per-motor. If payload doesn't contain motor id
            # yet, fall back to the general response_id key.
        ...
        try:
            # Send command
            self._ask('send', self.device, message_bytes)
            # Block on requested futures until they complete. 
            for f in wait_plan:
                f.result()
    ...
    def end_batch(self, blocking=True):
        ...
        try:
            # All commands in batch share the same blocking flag. This means
            # end_batch(blocking=True) waits for every response, which is safe
            # but may be slower than necessary for fire-and-forget commands like
            # motor_set_speed. A future optimisation could track per-command
            # blocking intent, but the current approach is simpler and correct.
    ...
    def search(self, timeout = 5, device_name=None, device_mac=None, card_color=None, card_serial=None):
        ...
        # Prevent calling from callback thread
        self._validate_thread("search")
        ...
        # Perform the scan inline (formerly in scan()). Results arrive via _scan_callback.
        try:
            self.scanning = True
            self._ask('scan', timeout_value, self._scan_callback, filters)
            while self.scanning:
                time.sleep(0.05)
            self._last_scan_filters = filters
        except KeyboardInterrupt:
            # Reset scanning state and shut down the BLE worker thread so the
            # process can exit cleanly on Ctrl+C.
            self.scanning = False
            self.logger.info("Scan interrupted by user")
            self._Background_thread_end()
            raise
        except Exception:
    ...
    def connect(self, device=None, *, device_name=None, device_mac=None, card_color=None, card_serial=None, device_notification_delay = DEFAULT_DEVICE_NOTIFICATION_DELAY):
        ...
        # Clear readiness signal for new connection
        self._first_notification_ready.clear()

        self.device = device
        self.connected = False
        try:
            self._ask('connect', device, self._device_callback, self._connect_callback, self._disconnect_callback)
            ...
            if self.version_rpc != RPC_VERSION:
                self.connected = False
                self._ask("disconnect", self.device)
                if self.version_rpc > RPC_VERSION:
                    msg = (
            ...
            # Ctrl+C can arrive during the polling loop, info(), or
            # notification setup — all are covered by this handler.
            self.connected = False
    ...
    def _wait_for_first_notification(self, timeout: float) -> bool:
        ...
        """
        return self._first_notification_ready.wait(timeout=timeout)
    ...
    def disconnect(self):
        ...
        self._validate_thread("disconnect")
        # If device is None we still want to trigger a shutdown of any remaining connections
        if self.device is not None:
            self._ask('disconnect', self.device)
        else:
            # Fallback: ask worker to close all (handles partial init or previous disconnect)
            self._ask('close_all')
